  ...
  <div v-else>
    <div class="list-box" v-for="project in projects" :key="project.id">
      <el-card class="list-item">
        ...
        <el-button icon="el-icon-office-building" @click="clickToDelete(project.id)">环境变量</el-button>
...
import {addProject, deleteProject, listProject} from "../api/project"
...
export default defineComponent({
  ...
  setup: () => {
    ...
    let addNewProject = reactive({
      name: '',
      desc: ''
    })

    async function clickToAdd() {
      await addProject({name: addNewProject.name, desc: addNewProject.desc})
      await loadProjects()
